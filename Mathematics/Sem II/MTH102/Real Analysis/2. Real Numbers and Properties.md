...
(a) If $S$ contains a largest element $s_{0}$ (that is, $s_{0}$ belongs to $S$ and $s ≤ s_{0}$ for all s ∈ S), then we call $s_{0}$ the maximum of $S$ and write $s_{0} = max S. 
(b) If S contains a smallest element, then we call the smallest element the minimum of S and write it as min S.